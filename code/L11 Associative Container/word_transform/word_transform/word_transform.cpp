...
{
	//the actual map works,this part is the heart of the program
...
//first arguement is the transformations file
//second is file to transform
void word_transform(ifstream &map_file, ifstream &input)
{
	auto trans_map = buildMap(map_file);	//store the transformations

	//for debugging the purpose print the map after its build
	cout << "Here is our transformations map:\n\n";
	...
	while (getline(input, text)){	//get a line of input
		...
		while (stream >> word){
			...
			//transform returns its first arguement or its transformation
			cout << transform(word, trans_map);		//print the output
		}
		cout << endl;		//done with the line of input
...
int main(int argc, char **argv)
{
	...
	if (!map_file)					//check that open succeeded
		throw runtime_error("no transfrom file");
